 ...
 */
template <typename T, uint8_t... Bytes> struct NBTValue {
  using TYPE = T;
 ...
 */
template <unsigned int N = 1, uint8_t... Bytes> struct NBTstream {
  static constexpr std::size_t N_VALUES{N};
  static constexpr std::size_t STREAM_LENGTH = sizeof...(Bytes);

  static constexpr uint8_t STREAM[STREAM_LENGTH]{Bytes...};

  explicit constexpr NBTstream<N, Bytes...>() {}
