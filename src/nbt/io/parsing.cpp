...
bool _INBTTypeParser::check_tag_id(NBT_STREAM_STD_ARGS, uint8_t tag) {
...
ParseResult _INBTTypeParser::read_name_size(NBT_STREAM_STD_ARGS,
  ...
  if (auto ret = read_short(strm, N, dest->name_length);
...
ParseResult _INBTTypeParser::read_name(NBT_STREAM_STD_ARGS, NBTType *dest) {
  ...
  // Parse name bytes
  if (auto ret = read_string(strm, N, &dest->name, dest->name_length);
      ret != ParseResult::SUCCESS)
    return ret;
...
ParseResult _INBTTypeParser::parse_type_header(NBT_STREAM_STD_ARGS, uint8_t tag,
                                               NBTType *dest) {
...
template <> ParseResult NBTShortParser::parse_payload(_NBT_STREAM_STRM_ARGS) {
  if (auto ret = read_short(strm, N, curr->val); ret != ParseResult::SUCCESS)
    return ret;
  flags |= ParseFlags::PAYLOAD | ParseFlags::PAYLOAD_SIZE;
  return ParseResult::SUCCESS;
}
...
template <> ParseResult NBTIntParser::parse_payload(_NBT_STREAM_STRM_ARGS) {
  if (auto ret = read_int(strm, N, curr->val); ret != ParseResult::SUCCESS)
    return ret;
  flags |= ParseFlags::PAYLOAD | ParseFlags::PAYLOAD_SIZE;
  return ParseResult::SUCCESS;
}
...
template <> ParseResult NBTLongParser::parse_payload(_NBT_STREAM_STRM_ARGS) {
  if (auto ret = read_long(strm, N, curr->val); ret != ParseResult::SUCCESS)
    return ret;
  flags |= ParseFlags::PAYLOAD | ParseFlags::PAYLOAD_SIZE;
  return ParseResult::SUCCESS;
}
...
ParseResult NBTStringParser::parse_payload(_NBT_STREAM_STRM_ARGS) {
  ...
  if (!has_flag(flags, ParseFlags::PAYLOAD_SIZE)) {
    if (ret = read_short(strm, N, curr->val_size); ret != ParseResult::SUCCESS)
      return ret;
    flags |= ParseFlags::PAYLOAD_SIZE;
  }

  // Parse string
  if (ret = read_string(strm, N, &curr->val, curr->val_size);
      ret != ParseResult::SUCCESS) {
...
// ----------------------------------------------------------------------------
/**
 * @brief Parse an float value from a NBT stream
 */
ParseResult NBTFloatParser::parse_payload(_NBT_STREAM_STRM_ARGS) {
  // Parse float bytes
  if (auto ret = read_int(strm, N, _bytes); ret != ParseResult::SUCCESS)
    return ret;

  // Convert it to a float value
  curr->val = *reinterpret_cast<float *>(&_bytes);
  flags |= ParseFlags::PAYLOAD | ParseFlags::PAYLOAD_SIZE;
  return ParseResult::SUCCESS;
}
// ----------------------------------------------------------------------------
/**
 * @brief Parse an double value from a NBT stream
 */
ParseResult NBTDoubleParser::parse_payload(_NBT_STREAM_STRM_ARGS) {
  // Parse float bytes
  if (auto ret = read_long(strm, N, _bytes); ret != ParseResult::SUCCESS)
    return ret;
